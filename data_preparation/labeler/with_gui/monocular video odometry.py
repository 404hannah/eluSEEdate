...
class VisualOdometry():
    def __init__(self, data_dir, video_path):
        ...
        cv2.resizeWindow("Matches", 920, 920) 
...
def process_video(data_dir, video_file):
    ...
    for i in tqdm(range(vo.num_frames), desc=f"Video: {video_file}", unit="frame"):
        ...
        if i > 0 and vo.px_frame is not None:
            ...
            # Visualization
            img_draw = cv2.drawMatches(vo.px_frame, kp1, curr_frame, kp2, good_matches, None, flags=2)
            
            text_str = f"{label_name}"
            ...
            cv2.putText(img_draw, text_str, (text_x, text_y), font, font_scale, (0, 0, 0), thickness)
            
            cv2.imshow("Matches", img_draw)
            cv2.waitKey(1)
