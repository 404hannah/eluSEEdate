...
export default function NoIntentScreen({ navigation }: NoIntentScreenProps) {
   ...
   */
  useEffect(() => {
    console.log('[NoIntent] Screen mounted');
    console.log('[NoIntent] Permission status:', permission?.granted ? 'granted' : 'not granted');
    
    const initModels = async () => {
      console.log('[NoIntent] Initializing models...');
      ...
      if (convlstmLoaded) {
        console.log('[NoIntent] ConvLSTM model initialized successfully');
      } else {
        console.log('[NoIntent] ConvLSTM model failed to load');
      }
      ...
      if (yoloLoaded) {
        console.log('[NoIntent] YOLO model initialized successfully');
      } else {
        console.log('[NoIntent] YOLO model failed to load');
      }
  ...
  const startCapture = useCallback(() => {
    ...
    console.log(`[NoIntent] Starting continuous capture at ${REALISTIC_CAPTURE_FPS} FPS...`);
  ...
  const stopCapture = useCallback(() => {
    ...
    console.log('[NoIntent] Capture stopped');
  }, []);
  ...
  const captureFrame = async () => {
    ...
    try {
      ...
      if (!photo) {
        console.log('[NoIntent] No photo returned');
      ...
      console.log(`[NoIntent] Frame captured in ${captureTime}ms`);
      ...
      if (photo.base64) {
        try {
          ...
          console.log(`[NoIntent] Frame decoded #${sequenceId}: ${decoded.width}x${decoded.height}, ${decoded.data.length} bytes`);
        } catch (decodeError: any) {
          console.warn('[NoIntent] Failed to decode image:', decodeError?.message);
          setDebugStatus(`Decode error: ${decodeError?.message || 'invalid frame'}`);
          return;
        }
      } else {
        // No base64 data available.
        console.warn('[NoIntent] No base64 data in photo');
      ...
      if (wasAdded) {
        ...
        setFrameCount(prev => {
          ...
          console.log(`[NoIntent] Frame ${newCount} added to buffer (${bufferCount}/${SEQ_LEN}) seq=${frameData.sequenceId ?? -1}`);
          return newCount;
        });
      ...
      console.error('[NoIntent] Frame capture error:', error?.message || error);
  ...
  const runInferenceWithPadding = async () => {
    ...
    try {
      ...
      console.log(
        `[TRACK][NoIntent][Run ${runTag}] strategy=${strategy} buffer=${buffer.getFrameCount()}/${SEQ_LEN} unique=${uniqueIds.length}/${frames.length}`
      );
      console.log(`[TRACK][NoIntent][Run ${runTag}] sequence=${sequenceIds.join(',')}`);
      ...
      setPredictionCount(prev => {
        ...
        console.log(`[NoIntent] Prediction #${newPredCount}: ${prediction.className} (${(prediction.confidence * 100).toFixed(1)}%)`);
        console.log(`[NoIntent] Latency: ${newMetrics.totalLatencyMs.toFixed(1)}ms`);
        return newPredCount;
      });
      
    } catch (error: any) {
      console.error('[NoIntent] Inference error:', error?.message || error);
  ...
  if (!permission) {
    console.log('[NoIntent] Permission not determined yet');
    return (
  ...
  // Permission denied
  if (!permission.granted) {
    console.log('[NoIntent] Permission denied');
    return (
      <SafeAreaView style={styles.container}>
  ...
  console.log('[NoIntent] Rendering camera view - permission granted');
  ...
  return (
    <View style={styles.container}>
      ...
      <CameraView
        ...
        enableTorch={false}
        onCameraReady={() => {
          console.log('[NoIntent] Camera is ready!');
          setIsCameraReady(true);
          setDebugStatus('Camera ready');
        }}
        onMountError={(error) => {
          console.error('[NoIntent] Mount error:', error);
